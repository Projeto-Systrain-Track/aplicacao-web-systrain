const dadosLinhas = {
  7: {
    ...
    descricao: 'vai ficar uma mensagem padrão de status. Tipo um "ta suave" "tem que dar atenção" "Crítico"',
    estacoes: 17,
    incidentes: 3,
    disponibilidade: '92%',
    cor: '#9B1B6D',
    metricas: {
      cpu: { valor: '57%', stat1: 'Pico 74%', stat2: 'Média 57%', serie: [46,48,52,55,59,62,60,58,61,57,56,57] },
      ram: { valor: '68%', stat1: 'Pico 79%', stat2: 'Média 68%', serie: [58,60,61,64,66,68,70,71,69,67,66,68] },
      latencia: { valor: '103 ms', stat1: 'Pico 146 ms', stat2: 'Média 103 ms', serie: [82,86,91,95,102,118,111,107,109,103,99,103] },
      incidentes: { valor: '3', stat1: 'Críticos 1', stat2: 'Médios 2', serie: [1,1,2,2,2,3,3,2,2,3,3,3] }
    }
  },
  9: {
    nome: 'Esmeralda',
    ...
    cor: '#00A88E',
    metricas: {
      cpu: { valor: '42%', stat1: 'Pico 61%', stat2: 'Média 42%', serie: [31,34,36,39,40,44,47,46,43,41,42,42] },
      ram: { valor: '61%', stat1: 'Pico 70%', stat2: 'Média 61%', serie: [51,54,55,58,60,63,65,64,62,61,60,61] },
      latencia: { valor: '84 ms', stat1: 'Pico 108 ms', stat2: 'Média 84 ms', serie: [76,74,78,80,82,85,89,87,86,83,82,84] },
      incidentes: { valor: '2', stat1: 'Críticos 0', stat2: 'Médios 2', serie: [0,1,1,1,2,2,1,1,2,2,2,2] }
    }
  },
  10: {
    nome: 'Turquesa',
    ...
    cor: '#0099AD',
    metricas: {
      cpu: { valor: '49%', stat1: 'Pico 67%', stat2: 'Média 49%', serie: [39,40,41,43,46,49,52,53,51,49,48,49] },
      ram: { valor: '64%', stat1: 'Pico 73%', stat2: 'Média 64%', serie: [55,57,58,60,61,64,67,68,66,65,64,64] },
      latencia: { valor: '91 ms', stat1: 'Pico 120 ms', stat2: 'Média 91 ms', serie: [79,80,82,86,88,94,97,100,96,94,92,91] },
      incidentes: { valor: '1', stat1: 'Críticos 1', stat2: 'Médios 0', serie: [0,0,0,1,1,1,1,1,0,1,1,1] }
    }
  }
};
...
function renderizarLinha() {
  ...
  const grupos = document.querySelectorAll('.map-line-group');

  for (let i = 0; i < grupos.length; i++) {
    const grupo = grupos[i];
    const numeroLinha = Number(grupo.dataset.line);

    if (numeroLinha === chave) {
...
function abrirModalMetrica(metrica) {
  ...
  document.getElementById('tituloModal').textContent = nomeMetrica + ' - Linha ' + chave + ' - ' + dados.nome;
      ...
      datasets: [
        {
          ...
          backgroundColor: metrica === 'incidentes' ? dados.cor + '55' : dados.cor + '22',
...
document.getElementById('modalMetrica').addEventListener('click', function (event) {
...
for (let i = 0; i < grupos.length; i++) {
  grupos[i].addEventListener('click', function () {
    const chave = Number(grupos[i].dataset.line);
    indiceLinha = ordem.indexOf(chave);
    renderizarLinha();
  });
}
